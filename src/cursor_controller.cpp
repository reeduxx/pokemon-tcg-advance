...
CursorController::CursorController(Cursor& cursor, BattleEngine& battle_engine) : m_cursor(cursor), m_battle_engine(battle_engine) {
...
void CursorController::update() {
	...
	if(m_cursor.mode() == CursorMode::HAND && m_cursor.position() != m_battle_engine.player_hand().get_card_pos(m_cursor.hand_idx())) {
	...
	if(bn::keypad::right_pressed()) {
		if(m_cursor.mode() == CursorMode::HAND) {
			...
			if(m_battle_engine.field().side() == Side::SIDE_PLAYER) {
				snap_to_zone(ZoneId::PLAYER_ACTIVE);
			}
			if(m_battle_engine.field().side() == Side::SIDE_OPPONENT) {
	...
	if(bn::keypad::down_pressed() && m_battle_engine.current_state() >= BattleState::BATTLE_START) {
		if(zone == ZoneId::PLAYER_ACTIVE) {
			...
			if(m_battle_engine.player_hand().card_count() > 0) {
				...
				m_cursor.set_hand_idx(0);
				bn::fixed_point pos = m_battle_engine.player_hand().get_card_pos(0);
...
void CursorController::snap_to_zone(ZoneId id, Side side) {
	...
	if(side == Side::SIDE_OPPONENT) {
		m_cursor.set_pos(m_battle_engine.field().get_zone(id).m_opponent_pos);
	} else {
		m_cursor.set_pos(m_battle_engine.field().get_zone(id).m_pos);
	}
}
